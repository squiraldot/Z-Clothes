  ...
  return (
    <main className="page wishlist-page">
      <div className="page-hero">
        ...
        <h1>Your Wishlist</h1>
        <p>{saved.length ? `${saved.length} piece${saved.length === 1 ? '' : 's'} waiting for you.` : 'Keep your favourites close. Tap the heart on any product to save it.'}</p>
      </div>
      {!saved.length ? (
        <div className="wishlist-empty">
          <Heart size={34} weight="regular" />
          <h2>Nothing saved yet.</h2>
          <p>Explore the collection and build your personal edit.</p>
          <Link href="/products" className="btn dark">Explore products →</Link>
        </div>
      ) : (
