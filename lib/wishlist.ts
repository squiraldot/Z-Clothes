...
export async function syncWishlist() {
  ...
  const missingRemote = localIds
    .filter((productId) => !remoteIds.includes(productId))
    .map((productId) => ({ user_id: user.id, product_id: productId }));
...
export async function toggleRemoteWishlist(productId: string) {
  ...
  const next = existing ? readWishlist().filter((id) => id !== productId) : [...readWishlist(), productId];
  setWishlist(next);
  return next;
}
