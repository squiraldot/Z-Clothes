...
export function CartDrawer({ open, onClose }: { open:boolean; onClose:()=>void }) {
  ...
  return <>
    ...
    <aside className={open?'cart-drawer open':'cart-drawer'} aria-label="Shopping bag" aria-hidden={!open}>
      <div className="cart-drawer-head"><div><span className="eyebrow dark">YOUR BAG</span><h2>{count} {count===1?'piece':'pieces'}</h2></div><button onClick={onClose} aria-label="Close bag"><X size={22}/></button></div>
